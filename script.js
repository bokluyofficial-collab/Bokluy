...
function clearSupabaseStorage() {
  try {
    // Supabase v2 stores keys like: sb-<projectRef>-auth-token
...
// ========================
// OWNER LOCK (ONLY YOU CAN ADD/EDIT/DELETE LISTINGS)
// Put YOUR Supabase user id here (UUID)
...
function makeDMRoomName(a, b) {
  // deterministic + unique, so the same two users share one DM
...
// Optional: when game changes on the form, clear item + autocomplete
...
function renderItems(items) {
  ...
  items.forEach((item) => {
    ...
    const fallbackImg = './images/logo.jpg'
    ...
    card.innerHTML = `
    <img src="${itemImage}" onerror="this.src='./products/images/placeholder.png'">

    <div class="listing-topline">
      <span class="listing-seller">👤 ${item.seller_name || "User"}</span>
      <span class="listing-time">⏱ ${timeAgo(item.created_at)}</span>
    </div>

    <div><strong>${item.name}</strong></div>
    <div>${item.game}</div>
    <div>$${Number(item.price).toLocaleString()} (TV: ${tradeValue})</div>
  ...
  `;
    ...
    <div class="contact-row">
      <div class="contact-text">Discord: ${item.discord}</div>
  ...
  `;
    ...
    // Show Message only if viewer is NOT the seller
...
function openListingDetails(item) {
  ...
  body.innerHTML = `
    ...
    <p><strong>Seller:</strong> ${item.seller_name || "User"}</p>
    <p><strong>Contact:</strong> ${item.contact_method === "discord" ? `Discord: ${item.discord || ""}` : "In-app chat"}</p>
    <p><strong>Notes:</strong> ${item.notes ? item.notes : "—"}</p>
  `
...
document.addEventListener("DOMContentLoaded", () => {
  const loginLink = document.getElementById("login-link");   // <a>
  const userMenu = document.getElementById("user-menu");     // wrapper
  const logoutBtn = document.getElementById("logout-btn");   // button
  ...
  logoutBtn?.addEventListener("click", async () => {
    try {
      // Prefer local scope so logout works even if revocation/network fails
      await supabase.auth.signOut({ scope: "local" });
    } catch (e) {
      console.warn("signOut failed (will clear local storage anyway):", e);
    } finally {
      clearSupabaseStorage();
      refreshUI(null);
      // reload listings so owner actions disappear
      try { await loadListings(); } catch {}
      window.location.href = "./auth.html?logged_out=1";
    }
  });

  // Add listing is available to any signed-in user
